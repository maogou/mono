...
func (l Logger) Trace(ctx context.Context, begin time.Time, fc func() (string, int64), err error) {
	...
	case err != nil && l.LogLevel >= gormlogger.Error && (!errors.Is(
		...
		sql, rows := fc()
		logger.Warn(
			"trace", zap.String("sqlerr", err.Error()), zap.String("elapsed", elapsedStr), zap.Int64("rows", rows),
			zap.String("sql", sql),
		)
	case elapsed > l.SlowThreshold && l.SlowThreshold != 0 && l.LogLevel >= gormlogger.Warn:
		...
		logger.Warn(
			"trace", zap.String("slow", slowLog), zap.String("elapsed", elapsedStr), zap.Int64("rows", rows),
			zap.String("sql", sql),
		)
...
func (l Logger) logger(ctx context.Context) *zap.Logger {
	...
	if ctx != nil {
		...
		zl := ctx.Value(zlog.CtxLoggerKey)
